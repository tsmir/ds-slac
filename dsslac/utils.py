...
import numpy as np
def sample_reproduction(latent_model, device, state, action):
    state = torch.tensor(state, dtype=torch.float32, device=device)/255.0
    action = torch.tensor(action, dtype=torch.float32, device=device)
    feature_ = latent_model.encoder(state)
...
def generate_taus(batch_size, N, N_hat, device, risk_level=1.0):
    taus = torch.rand(
            batch_size, N, device=device)

    taus_hat = torch.rand(
            batch_size, N_hat, device=device)
    
    taus = 1 - risk_level + risk_level * taus
    taus_hat = 1 - risk_level + risk_level * taus_hat

    return taus, taus_hat
